...
class Floor(Sprite):
    ...
    def update(self):
        ...
        if self.rect[0] <= self.floor_start - self.screen_width:
            self.rect[0] = self.floor_start
 
            
class Pipe(Sprite):
    
    def __init__(self, config, pipe_start, pipe_size, upsidedown):
        pygame.sprite.Sprite.__init__(self)
        ...
        self.rect[3] = pipe_size
        
        # otočí pipe
        if upsidedown:
            self.image = pygame.transform.flip(self.image, False, True)
            self.rect[1] = 0
            self.image = pygame.transform.scale(self.image, (config.PIPE_WIDTH, pipe_size))
        else:
            self.rect[1] = config.SCREEN_HEIGHT - pipe_size
        self.mask = pygame.mask.from_surface(self.image)
...
if __name__ == '__main__':
    ...
    pipe_group = Group([generate_pipe_pairs(config, config.SCREEN_WIDTH), 
                        generate_pipe_pairs(config, 2*config.SCREEN_WIDTH)])
    ...
    while True:
        ...
        if is_playing:
            ...
            screen.blit(score_txt, (config.SCREEN_WIDTH - score_txt.get_width() - 20,
                                    0 + score_txt.get_width() // 2))
            
            if pygame.sprite.groupcollide(
                bird_group, floor_group, False, False, pygame.sprite.collide_mask) or pygame.sprite.groupcollide(
                bird_group, pipe_group, False, False, pygame.sprite.collide_mask):
                is_playing = False
        else:
            game_over_txt = game_over_font.render('Game Over', True, (255, 0, 2))
            score_txt = score_font.render(str(score), True, (255, 255, 255))
            screen.blit(game_over_txt, (config.SCREEN_WIDTH // 2 - game_over_txt.get_width() // 2, 
                                        config.SCREEN_HEIGHT // 2 - game_over_txt.get_height() // 2))
            screen.blit(score_txt, (config.SCREEN_WIDTH // 2 - score_txt.get_width() // 2, 
                                        config.SCREEN_HEIGHT // 2 - score_txt.get_height() // 2 + 60))
